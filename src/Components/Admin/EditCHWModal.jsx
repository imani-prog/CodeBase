...
const EditCHWModal = ({ chw, isOpen, onClose, onSave }) => {
  ...
  const renderField = ({ label, name, type = "text", icon: FieldIcon, required, options }) => (
    <div>
      <label className="block text-sm font-medium text-gray-700 mb-2">
        {label} {required && <span className="text-red-500">*</span>}
      </label>
      <div className="relative">
        {FieldIcon && <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
        ...
        {options ? (
          ...
          <input
            ...
            className={`w-full ${FieldIcon ? "pl-10" : "px-4"} pr-4 py-3 border ${
              errors[name] ? "border-red-300 focus:ring-red-500" : "border-gray-300"
            } rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-700 focus:border-transparent transition-all`}
            placeholder={label}
          />
        )}
        {errors[name] && (
          ...
          <div className="relative px-8 py-6 bg-blue-950 text-white">
            ...
            <div className="flex items-center space-x-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-white/90">Update Community Health Worker information</p>
              </div>
            </div>
          </div>
