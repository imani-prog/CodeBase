...
const HospitalCarousel = () => {
  ...
  const visible = hospitalTestimonials.slice(index, index + visibleCount);
  const prev = () => setIndex(i => (i - visibleCount + hospitalTestimonials.length) % hospitalTestimonials.length);
  const next = () => setIndex(i => (i + visibleCount) % hospitalTestimonials.length);
